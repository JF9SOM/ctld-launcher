...
4. In the "Connection" section, select the serial port your rig/rotator is connected to, and the baud rate configured on the radio itself. If you plug in the cable after opening the app, click the refresh button next to the port dropdown to rescan.
   - For USB-connected devices, turn on "Auto-start when this USB device is connected" to have this profile start automatically whenever that USB device is plugged in, and stop automatically when it's unplugged (connect the device and select its port first, then turn this on). This only works while ctld-launcher itself is running — it doesn't start on login by itself; combine it with "Start at login" in the left sidebar if you also want that.
