...
4. 「接続」欄で、リグ/ローテーターを繋いだシリアルポートと、無線機側で設定されている通信速度を選びます。後からケーブルを挿した場合は、ポート欄横の再検出ボタンで一覧を更新できます。
   - USB接続の場合、「このUSBデバイス接続時に自動起動」をオンにすると、以後そのUSBデバイスを挿すと自動的にこのプロファイルが起動し、抜くと自動的に停止するようになります(先にデバイスを接続してポートを選んでからオンにしてください)。この機能はctld-launcher自体が起動している間のみ動作します(PCにログインしただけでは動作しません。ログイン時からデバイス非依存で自動起動したい場合は左サイドバーの「ログイン時に自動起動」と組み合わせてください)。
