...
- 左上のバージョン表示のすぐ下に、同梱しているHamlibのバージョンも表示されます。新しいctld-launcherが公開されると、その下に「↑ vX.Y.Zが利用可能です」という緑色のリンクが表示されるので、クリックしてダウンロード・インストールできます。更新完了後は再起動の確認が表示されます(稼働中のプロファイルを止めたくない場合は「後で」を選べます。更新ファイル自体は既に適用済みなので、次に手動で終了・起動し直したときに新バージョンになります)。
