...
`ctld-launcher.dmg`をダウンロードして開き、中の`ctld-launcher.app`をApplicationsフォルダなどにコピーしてください。初回起動時に開発元を確認できない旨の警告が表示され、削除を促されますが、**削除せずに閉じてください**。続けて、Appleメニュー→「システム設定」→「プライバシーとセキュリティ」を開き、下の方に表示される「"ctld-launcher"は開発元を確認できないため使用がブロックされました」といった項目の「このまま開く」ボタンをクリックし、確認ダイアログで「開く」を選んでください。これはApple公証を行っていないための表示で、2回目以降は通常通り起動できます。
...
4. 「接続」欄で、リグ/ローテーターを繋いだシリアルポートと、無線機側で設定されている通信速度を選びます。後からケーブルを挿した場合は、ポート欄横の再検出ボタンで一覧を更新できます。
   - USB接続の場合、「USB接続時にこのプロファイルを自動起動・切断時に自動停止」をオンにすると、以後そのUSBデバイスを挿すと自動的にこのプロファイルが起動し、抜くと自動的に停止するようになります(先にデバイスを接続してポートを選んでからオンにしてください)。この機能はctld-launcher自体が起動している間のみ動作します(PCにログインしただけでは動作しません。ログイン時からアプリ自体を自動起動したい場合は左サイドバーの「スタートアップ時に起動」と組み合わせてください)。
...
- 左側のプロファイル一覧、各プロファイル名の右端にあるON/OFFトグルをONにすると、アプリ起動時にそのプロファイルが自動的に起動します。左サイドバー下部の「スタートアップ時に起動」をオンにすると、PCログイン時にこのアプリ自体がトレイに常駐起動します。
- アプリを起動する(アイコンをクリックする、または`ctld-launcher`コマンドを実行する)と、通常は設定画面がすぐに開きます。ログイン時の自動起動の場合のみ、設定画面を開かずトレイに常駐した状態で起動します。
- ウィンドウを閉じてもアプリは終了せず、システムトレイに常駐します(設定画面左下の「トレイに最小化」ボタンでも同様に隠せます)。トレイアイコンを左クリックすると設定画面が再表示され、右クリックするとプロファイルごとのワンクリック起動/停止・設定画面を開く・終了のメニューが表示されます。アプリを完全に終了したい場合は、トレイメニューの「終了」か、設定画面左下の「ctld-launcherを終了」ボタンを使ってください。
- 左上のコンボボックスで、表示言語を英語/日本語に切り替えられます(再起動不要)。
- 左上のバージョン表示のすぐ下に、同梱しているHamlibのバージョンも表示されます。さらにその下の「アップデートを確認」リンクは、いつでもクリックしてGitHub上の最新版を確認できます(起動時にも自動で1回チェックされます)。新しいバージョンが見つかると「↑ vX.Y.Zが利用可能です」に表示が変わるので、もう一度クリックするとダウンロード・インストールできます。更新完了後は再起動の確認が表示されます(稼働中のプロファイルを止めたくない場合は「後で」を選べます。更新ファイル自体は既に適用済みなので、次に手動で終了・起動し直したときに新バージョンになります)。
