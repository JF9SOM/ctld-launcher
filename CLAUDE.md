...
- **サイドバーの「アップデートを確認」ボタンが常に「確認できませんでした」になる**(実機テストで発覚、Windows。原因は実機側で未特定): `fetch_latest_release()`は失敗理由を握りつぶして`None`を返す設計だったため(起動時の自動チェックを静かに失敗させるためにあえてそうしている)、手動チェック失敗時もユーザーには「インターネット接続を確認してください」としか出せず、実際の原因(DNS失敗なのかTLSなのか等)が全く分からなかった。`core/app_update.py`に例外を握りつぶさない`_fetch_latest_release_or_raise()`を切り出し、`fetch_latest_release()`はそれを`try/except`で包むだけの薄いラッパーに(公開契約・既存テストは無変更)。`UpdateCheckWorker.not_found`シグナルに`str`(失敗理由)を追加し、手動チェック(`manual=True`)時のみ`_on_update_check_not_found()`のダイアログに実際のエラー内容と`GITHUB_RELEASES`への手動確認リンクを併記するようにした(自動チェックは従来通り完全に無言のまま)。次回同じ症状が出た場合はこのダイアログの内容で原因を特定できる。
