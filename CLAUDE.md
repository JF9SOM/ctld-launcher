...
- **【mac開発環境固有・アプリ本体のバグではない】devソースを起動してもウィンドウもトレイアイコンも一切表示されないことがある**(Ubuntuからmacへ開発環境移行後、実機テストで発覚): 原因は2つ重なっていた。(1) `core/single_instance.py`の`SingleInstanceGuard`は正常終了しなかった残留プロセス(クラッシュ・強制終了等)にロックを握られると、次回起動が「既に起動しています」ダイアログすら出さずに何も表示しなくなることがある(ロック保持元自身が(2)の問題でトレイアイコンを出せていなかったため、警告ダイアログが出ても気づきにくい状態が連鎖していたと見られる)。dev起動前に`ps aux | grep ctld_launcher`で残留プロセスがないか確認するか、devランチャー側で自動killするのが実用的な対策。(2) デスクトップに自作した簡易devランチャー`.app`(リポジトリ管理外、個人の利便性のためのラッパー)のシェルスクリプトが`exec ctld-launcher`を使っていたため、shebang経由の1回目のexec(`/bin/zsh`起動時)に続けて同一PIDへの2回目のexecが発生し、macOSのRunningBoard/Control Center側のプロセス世代追跡が`process has mismatched pid version`エラーで壊れて、ステータスバー(トレイ)アイテムの登録がXPCエラー(`scene activation failed`)で永続的に失敗していた。`exec`を使わず素直に子プロセスとして起動する(スクリプト末尾を`exec ctld-launcher`ではなく`ctld-launcher`にする)ことで解消。**パッケージ版(dmg)は無関係**: `scripts/ctld-launcher.spec`が生成するCFBundleExecutableはPyInstaller製のネイティブブートローダー(コンパイル済みバイナリ)であり、シェルスクリプト内での追加execという構造自体が存在しないため、この不具合の影響を受けない。
- **【mac開発環境固有】devソースで実機テスト中、機種一覧にFTX-1(新しめのYaesu機)が出てこない**: `core/hamlib_locator.py`はdevモード(`sys._MEIPASS`なし)では素直にPATH上の`rigctld`/`rotctld`を使う設計(意図的な仕様、パッケージ版は同梱版を優先)。ところが上記のトレイアイコン不具合と同根で、LaunchServices経由(`.app`のダブルクリックや`open`)で起動したプロセスは`~/.zshrc`等で追加したPATHを一切引き継がない(素の`/usr/bin:/bin:/usr/sbin:/sbin`のみ)。devランチャー`.app`のスクリプトが`source .venv/bin/activate`で venv の bin を先頭に足すだけなので、それ以外のPATHは素のままになり、たまたまPATHに残っていた**MacPorts版の`/opt/local/bin/rigctld`(Hamlib 4.5.5、2023年ビルド、FTX-1未対応)**が見つかってしまっていた。`.github/hamlib-version.txt`が指す4.7.2自体は正しくビルドできており(姉妹プロジェクト[FBSAT59](../FBSAT59)の`.venv/bin/rigctld`が同じ4.7.2をセルフビルド済みで、FTX-1にも対応済みだったので、そちらのvenvをPATHの先頭に追加して解決)、リポジトリ直下の`hamlib-bundle/`(CI成果物をローカルに置いたもの)は`libhamlib.4.dylib`のdyldエラーで直接実行できなかった(PyInstallerバンドル内の`_MEIPASS/hamlib/`という特定の配置を前提にリンクされているため、リポジトリ直下からの単体実行は未対応)。**パッケージ版(dmg)は無関係**: そちらは`_MEIPASS/hamlib/`同梱版を最優先で使うため、PATHにもMacPortsの古いHamlibにも依存しない。
- **稼働中デーモンへのヘルスチェック/接続テストが、FTX-1のような機種で他クライアント(FBSAT59等)の周波数書き込みを黙って別VFOへ誤誘導することがある**(姉妹プロジェクトFBSAT59とのFO-29/FTX-1実機調査で発覚、2026-08-17): `_run_health_checks()`(5秒間隔)と、稼働中プロファイルへの「Test Connection」ボタンは、どちらも旧`build_test_command_via_daemon()`でHamlib組み込みのNET rigctl/rotctlクライアント(`rigctl -m 2 -r host:port f`)をサブプロセスとして起動し、稼働中のrigctld/rotctldへ別のTCPクライアントとして接続していた。このクライアントは、Hamlibの汎用`rig_open()`が接続直後に自動送信する「v」(get_vfo)問い合わせを、ctld-launcher自身のコードには一切現れない形で(Hamlibライブラリ内部の暗黙の挙動として)発生させる。サーバー側の`rig_get_vfo()`はこの問い合わせに対し、無線機に実際に`VS;`(アクティブVFO問い合わせ)を送って結果を返すだけでなく、**rigctldプロセス全体で共有される`current_vfo`の値をその結果で上書きする**(Hamlib `src/rig.c`のVFOキャッシュ実装。4.7.1/4.7.2とも同一コードで、Hamlibのバージョンは無関係と確定済み)。FTX-1のようなクロスバンド/split構成で継続的にDoppler追尾するリグは、無線機の物理的な表示VFOが運用中にSub側を指すことがある既知のファームウェア癖を持つため、ヘルスチェックのタイミングがそこに重なると、rigctldが管理する`current_vfo`が恒久的にSubへ書き換わってしまい、以降**他のどのクライアント(FBSAT59・GPredict・WSJT-X等)がVFO指定なしで送る「F」書き込みも全てSub側へ誤配送される**という、ctld-launcher自身の画面には一切現れない副作用を生んでいた。
  - **修正**: `core/process_manager.py`に`probe_daemon()`を新設し、rigctl/rotctlサブプロセスを一切経由せず、生ソケットで直接プレーンな「f」(rig)/「p」(rotator)をrigctld/rotctldへ送るだけの方式に変更(`build_test_command_via_daemon()`は削除)。Hamlibソース(`src/rig.c`)を確認した結果、`rig_get_freq()`は`current_vfo`を読むだけで一切書き換えないことを確認済み(FBSAT59自身がLock機能で日常的に送っている「f」/「i」と全く同じ安全なコマンド)。ヘルスチェック・Test Connectionボタンの両方をこの方式に切り替え、副次的に対象プロファイルのrigctl/rotctl実行ファイルを探す必要もなくなった。
  - **教訓**: 「稼働中デーモンへの疎通確認」のような一見無害な監視機能でも、それが使うクライアントライブラリが接続の副作用として何を送っているかまでは意識されにくい。特にHamlibのように複数のTCPクライアントが単一の共有リグ状態(`current_vfo`)を扱うプロトコルでは、あるクライアントの「読み取りのつもり」の操作が、無線機の物理状態と絡んで他の全クライアントの動作に影響することがある。調査の詳細な経緯(Hamlib 4.7.1/4.7.2ソース比較・rigctldプロトコル解析等)はFBSAT59のCLAUDE.mdを参照。
