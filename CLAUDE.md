...
- `core/usb_watch.py` — Linuxのudev+systemd(`ACTION=="add"`でサービス起動、`"remove"`で停止)と同等の体験を、macOS/Windowsを含む3プラットフォーム共通のコードで実現するための実装。
  ...
  - 既存の`Profile.auto_start`(アプリ起動時に自動起動、プロファイル一覧の各行右端のチェックボックスで設定)とは独立した設計。両方オンにしても害はない(`_start_profile()`/`_stop_profile()`はどちらも「既に希望の状態ならno-op」なので二重起動しない)。
  - **文言の混乱を避けるための整理**(実機テスト中のユーザーフィードバックで発覚): 当初「自動起動」という単語をサイドバー(ログイン時)・フォーム(アプリ起動時)・接続欄(USB接続時)の3箇所で使い回していたところ、どれが何を指すか分からず混乱を招いた。整理後は次の通り、単語の重複を避けている。
    ...
    - プロファイル一覧の各行右端のラベルなしチェックボックス(ツールチップのみで説明、`_sidebar_autostart_checkboxes`): アプリ起動時にそのプロファイルも起動。以前はフォーム上部に「自動起動」という名前付きチェックボックスとして存在していたが、(1) 1プロファイルずつフォームを開かないと状態を確認できない、(2) 「自動起動」という単語がサイドバー①と衝突する、という2つの理由で一覧行のチェックボックスに移動した。
